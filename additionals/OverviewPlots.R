...
# tmp.path <- "C:/Users/clehr/Documents/Tmp/"
############################
############################
...
path.legends <- paste(path$w[path$n == "plot.p"], "Legends", sep = "")
# collect data from wikiplot png?s
path.wiki.pngs <- paste(path$w[path$n == "plot.p"], year_i, "/", sep = "")
# vector for selecting the corresponding wikiplot
names.wikiplots <- read.table(file = "N:/sparc/LTO/R_database/database_R/settings/wikiplots_variables.csv", header = TRUE, sep = ",")
# names.wikiplots
...
#  for (i in 22) {
    ...
    for (j in 1:length(all.years)) { #years[[k]])) {
      ...
      # k == 1: "met"
      if (k == 1) {
        #if (names(lv1.data)[i] == "RadNet") {
         # ii <- 0
        #} else {
          ii <- which(names(dummy) == dummy.names.met[i])
       # }
        # if (names(lv1.data)[i] == "Prec") {
        #   ii <- which(names(dummy) == "prec")
        # } else if (names(lv1.data)[i] == "RadNet") {
        #   ii <- 0
        # } else if (names(lv1.data)[i] == "Vwind_200") {
        #   ii <- which(names(dummy) == "wind_v_200")
        # } else if (names(lv1.data)[i] == "Dirwind_sd_200") {
        #   ii <- which(names(dummy) == "wind_sddeg_200")
        # } else if (names(lv1.data)[i] == "Dirwind_200") {
        #   ii <- which(names(dummy) == "wind_deg_200")
        # } else if (names(lv1.data)[i] == "Dsn_a") {
        #   ii <- which(names(dummy) == "Dsn")
        # } else if (names(lv1.data)[i] == "Dsn_b") {
        #   ii <- which(names(dummy) == "Dsn")
        # } else if (names(lv1.data)[i] == "E2_sn") {
        #   ii <- which(names(dummy) == "E2_sn_v_0")
        # } else if (names(lv1.data)[i] == "Tair_5") {
        #   ii <- which(names(dummy) == "Tair_4")
        # } else {
        #   ii <- which(names(dummy) == names(lv1.data)[i])
        # }
      }

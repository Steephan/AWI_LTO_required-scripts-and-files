...
for (jahro in jahr){
  if(zack==2){
    db.Ts.lvl1 <-read.table(paste0(path$w[path$n=="LV1.p"],"SaSoil2012/00_full_dataset/SaSoil2012_",jahro,"_lv1.dat"),sep=",",dec=".",header=T, fill = TRUE)
  ...
  if(zack==2){
    ...
    png(paste(path$w[path$n=="plot.p"],jahro,"/SaSoil2012_airt_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)
  ...
  if(zack==2){
    ...
    png(paste(path$w[path$n=="plot.p"],jahro,"/SaSoil2012_temp_center_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)
  ...
  if(zack==2){
    ...
    png(paste(path$w[path$n=="plot.p"],jahro,"/SaSoil2012_temp_rim_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)
    par(mar=c(1,5,1,1),omi=c(0,0,0,0))
    plot(as.numeric(strptime(db.Ts.lvl1$UTC[tair_gut],format="%Y-%m-%d %H:%M")),db.Ts.lvl1$Ts_0[tair_gut], pch = 20,# cex.lab=1.7, cex.axis=1.5,   # albedo from file
         xlim=xxlim, ylim=c(-30,25), xlab="", ylab = "",xaxt="n", yaxt="n",type="n", cex.axis=3)
    #plot_maintenance(jahr)
  ...
  if(zack==2){
    ...
    png(paste(path$w[path$n=="plot.p"],jahro,"/SaSoil2012_Dsn_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)
    par(mar=c(1,5,1,1),omi=c(0,0,0,0))

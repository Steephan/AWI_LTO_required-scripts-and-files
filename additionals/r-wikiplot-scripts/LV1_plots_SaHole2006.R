...
for (jahro in run.year){
  ...
  png(paste(p.1$w[p.1$n=="plot.p"],jahro,"/SaHole2006_trompete_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)
  par(mar=c(1,8,1,1),omi=c(0,0,0,0))
  plot(c(stats.db[1,]),y.values,type="n",xlim=c(-16,8),ylim=c(-26,3),
  ...
  png(paste(p.1$w[p.1$n == "plot.p"],jahro,"/SaHole2006_temperature_",jahro,".png",sep=""), width = p.width, height = p.height, pointsize = 8)
  ...
  for(ll in seq(-40,30,10)){abline(h = ll,col = "gray80")} # horizontal lines

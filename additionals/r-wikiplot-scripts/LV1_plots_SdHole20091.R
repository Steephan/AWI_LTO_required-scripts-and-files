...
for (jahro in jahr){
#jahro<- 2015
  db.bahole.lvl1<-read.table(paste(path$w[path$n=="LV1.p"],"SdHole2009/SdHole2009_",jahro,".dat",sep=""),sep=",",dec=".",header=T, fill = TRUE)
  db.bahole.lvl2<-db.bahole.lvl1
  for(val in 2:25){# set data to NA if flag is not 0
  db.bahole.lvl2[which(as.numeric(db.bahole.lvl2[,(val*2)+1])>=1),(val*2)]<-NA
  }
  db.bahole.extra<-db.bahole.lvl2[,c(1,c(2:25)*2)]

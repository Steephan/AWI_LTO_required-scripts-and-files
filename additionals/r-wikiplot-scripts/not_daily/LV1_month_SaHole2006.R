...
out.path<-"N:/geo5/SoilData/data/level1/SaHole2006/temperature/month/"
...
data.temp<-read.table(paste(in.path,files.temp[i],sep=""),header=F,skip=3,dec=".",sep="\t")#[,c(1,2:25)]
data.temp[,1]<-as.POSIXct(data.temp[,1],format='%Y-%m-%d', tz = "UTC")
...
write.table(tem[,c(2:26)],paste(out.path,"SaHole2006_month_",2005+i,".dat",sep=""),quote = F,dec=".",sep=",",row.names=F)

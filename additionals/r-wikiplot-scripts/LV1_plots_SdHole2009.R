...
jahr=c(2009:2017)#2016)
#jahr=c(2015)
months <- c("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12")
Months <- c("Jan", " Feb", "Mar", "Apr", "May", "Jun","Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
soil.cols<-colorRampPalette(c("seagreen4","palegreen3","yellow3","khaki","sandybrown","peru","mistyrose3","peachpuff4"))(260) #100m
...
for (jahro in jahr){
#jahro<- 2015
  db.bahole.lvl1<-read.table(paste(path$w[path$n=="LV1.p"],"SdHole2009/00_full_dataset/SdHole2009_",jahro,"_lv1.dat",sep=""),sep=",",dec=".",header=T, fill = TRUE)
...
#   j.mean<-apply(db.bahole.extra[,2:25],2,FUN=mean, na.rm=TRUE)
  ...
  y.values<-c(0,-1*(c(0.4,0.8,1.2,1.6,2,2.5,3,4,5,7,8,11,13,15,20,30,40,50,60,70,80,90,100)))

  png(paste(path$w[path$n=="plot.p"],jahro,"/SdHole2009_trompete_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)
  par(mar=c(1,8,1,1),omi=c(0,0,0,0))
  plot(c(stats.db[1,]),y.values,type="n",xlim=c(-20,8),ylim=c(-100,5),
       xlab="", ylab = "",xaxt="n", yaxt="n",cex.axis=3)
  for(ll in seq(-100,0,10)){abline(h=ll,col="gray80")} # horizontal lines
  for(pp in seq(-20,15,2.5)){lines(c(pp,pp),c(-100,1),col="gray80")} # vertical lines
  for(qqq in 1:12){lines(c(stats.db[qqq,1:24]),y.values,col=mon.cols[qqq],lwd=2)}
...
#   lines(j.mean,y.values,col="purple",lwd=3)
  ...
  png(paste(path$w[path$n=="plot.p"],jahro,"/SdHole2009_",jahro,".png",sep=""),width=p.width,height=p.height,pointsize=8)

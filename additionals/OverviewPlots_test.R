...
# vector of datasets
dataset <- c("met", "soil", 
             "BaHole2009", "BaMet2009","BaSnow2013", "BaSoil2009", "BaSoil2017", 
             "TVCSoil2016",
...
for (year_i in run.year) {
  ...
  years <- list(2009:year_i, 2009:year_i,
              2009:year_i, 2009:year_i, 2013:year_i, 2009:year_i,2017:year_i,
              2016:2019, 
              2009:year_i,2012:year_i, 2009:year_i)
  ...
  # k: 1 and 2 Pangaea data sets
  if (k <= 2) {
    if(location == "Samoylov"){
      
      lv1.data <- read.table(paste(p.1$w[p.1$n == "LV2.p"], "Samoylov/ESSD_Pangaea/timeseries/LV1/", station, "_lv1_", year_i, ".dat", sep = ""),
                             sep = ",", dec = ".", header = T, fill = TRUE)
      pdf.path <- paste(p.1$w[p.1$n == "plot.p"], "OverviewPlots/LV1/", year_i,"/Pangaea_Sa_", station, "_", year_i, ".pdf", sep = "")
    }else{# Bayelva
      lv1.data <- read.table(paste(p.1$w[p.1$n == "LV2.p"], "Bayelva/ESSD_Pangaea/LV1/", station, "_lv1_", year_i, ".dat", sep = ""),
                             sep = ",", dec = ".", header = T, fill = TRUE)
      pdf.path <- paste(p.1$w[p.1$n == "plot.p"], "OverviewPlots/LV1/", year_i,"/Pangaea_Ba_", station, "_", year_i, ".pdf", sep = "")
    }
    ############################
    # pdf path
  ...
  y.lab[grep("Tsn", names(lv1.data))] <- "°C"
  y.lab[grep("Tgs", names(lv1.data))] <- "°C"
  ...
  y.lab[grep("ice", names(lv1.data))] <- "%"
  y.lab[grep("Ts", names(lv1.data))] <- "°C"
  y.lab[grep("water", names(lv1.data))] <- "%"
  y.lab[grep("rho", names(lv1.data))] <- "kg / m^3"
  y.lab[grep("wind_v", names(lv1.data))] <- "m / s"
  y.lab[grep("wind_deg", names(lv1.data))] <- "deg"
  # Pangaea
  y.lab[grep("Cond", names(lv1.data))] <- "S / m"
  y.lab[grep("Vwc", names(lv1.data))] <- "m^3 / m^3"# "%"
  # y.lab
  ...
  lv1.var.cat[grep("wind_v", names(lv1.data))] <- "wind_v"
  lv1.var.cat[grep("wind_deg", names(lv1.data))] <- "wind_deg"
  lv1.var.cat[grep("wind_sddeg", names(lv1.data))] <- "wind_sddeg"
  lv1.var.cat[grep("Dsn", names(lv1.data))] <- "Dsn"
  lv1.var.cat[grep("G", names(lv1.data))] <- "G"
  lv1.var.cat[grep("PA", names(lv1.data))] <- "PA"
  lv1.var.cat[grep("WL", names(lv1.data))] <- "WL"
  ...
  if (station == "SaSnow2012") {
    lv1.var.cat[grep("Tsn", names(lv1.data))] <- "Tsn"
    lv1.var.cat[grep("Tgs", names(lv1.data))] <- "Tgs"
    lv1.var.cat[grep("ice", names(lv1.data))] <- "ice"
    lv1.var.cat[grep("water", names(lv1.data))] <- "water"
    lv1.var.cat[grep("rho", names(lv1.data))] <- "rho"
    lv1.var.cat[grep("SWE", names(lv1.data))] <- "swe"
  }
  ...
  if (station == "met") {
    ...
    lv1.var.cat[grep("Prec", names(lv1.data))] <- "prec"
    lv1.var.cat[grep("prec", names(lv1.data))] <- "prec"
    lv1.var.cat[grep("Vwind", names(lv1.data))] <- "wind_v"
    lv1.var.cat[grep("Dirwind", names(lv1.data))] <- "wind_deg"
    lv1.var.cat[grep("Dirwind_sd", names(lv1.data))] <- "wind_sddeg"
    lv1.var.cat[grep("Vwind_max", names(lv1.data))] <- "wind_v_max"
    lv1.var.cat[grep("Vwind_min", names(lv1.data))] <- "wind_v_min"
    lv1.var.cat[grep("Dsn_a", names(lv1.data))] <- "Dsn_a"
    lv1.var.cat[grep("Dsn_b", names(lv1.data))] <- "Dsn_b"
  }
  # special cases Pangaea soil
  if (station == "soil" & location ==  "Bayelva") {
    lv1.var.cat[grep("Cond", names(lv1.data))] <- "Cond"
  ...
  # special cases Pangaea soil
  if (station == "soil" & location ==  "Samoylov") {
    
    lv1.var.cat[grep("Ts_center", names(lv1.data))] <- "Ts_center"
    lv1.var.cat[grep("Ts_slope", names(lv1.data))] <- "Ts_slope"
    lv1.var.cat[grep("Ts_rim", names(lv1.data))] <- "Ts_rim"
    lv1.var.cat[grep("Ts_icewegde", names(lv1.data))] <- "Ts_icewegde"
    lv1.var.cat[grep("Ts_borehole", names(lv1.data))] <- "Ts_borehole"
    lv1.var.cat[grep("E2_center", names(lv1.data))] <- "E2_center"
    lv1.var.cat[grep("E2_slope", names(lv1.data))] <- "E2_slope"
    lv1.var.cat[grep("E2_rim", names(lv1.data))] <- "E2_rim"
    lv1.var.cat[grep("Cond_center", names(lv1.data))] <- "Cond_center"
  ...
  if (station == "soil" & location ==  "Samoylov") {
    dummy.names.soil <- names(lv1.data)
    dummy.names.soil <- sub(pattern = "Ts_icewedge", replacement = "Ts_icewegde", dummy.names.soil)
  }
  ###.............................................................................
  ...
  for (i in seq(2, ncol(lv1.data), by = 2)) {
    ...
    # for Pangaea selection data set
    # select row index of dataset k and variable i in names.wikiplots table
    if ((station == "met") & (location == "Samoylov") & (lv1.var.cat[i] == "Dsn")) {
      ind.wiki.plots <- intersect(which(names.wikiplots$dataset == station), which(names.wikiplots$variable_category == lv1.var.cat[i]))
    } else if((station == "met") & (location == "Samoylov") & (lv1.var.cat[i] == "prec")) {
      ind.wiki.plots <- intersect(which(names.wikiplots$dataset == station), which(names.wikiplots$variable_category == lv1.var.cat[i]))[2]
    } else if((station == "met") & (location == "Samoylov")) {
      ind.wiki.plots <- rev(intersect(which(names.wikiplots$dataset == station), grep(lv1.var.cat[i], names.wikiplots$variable_category)))[1]
    } else {
      ind.wiki.plots <- intersect(which(names.wikiplots$dataset == station), grep(lv1.var.cat[i], names.wikiplots$variable_category))}
    ...
    for (j in 1:length(all.years)) {
      ...
      if (k == 1) {
        ...
        dummy <- read.table(paste(p.1$w[p.1$n == "LV2.p"],"Samoylov/ESSD_Pangaea/timeseries/LV1/", sparc.dataset, "_lv1_", year, ".dat", sep = ""),
                            sep = ",", dec = ".", header = T, fill = TRUE)
    ...
    if ((ii == 0) | (sum(!is.na(long.lv1.data[ind.long, ii])) == 0)) {
      ...
      if(colnames(lv1.data[i])=="prec"){
        ...
        axis(4, at = seq(0,10,2), labels = seq(0, 20, 4), las = 1, col.axis = "#ffaeb6")
      ...
      abline(v = c(long.lv1.data$UTC[which(paste0(run.year,"-01-01 01:00:00 UTC")==long.lv1.data$UTC)],
                   long.lv1.data$UTC[which(paste0(run.year,"-12-31 23:30:00 UTC")==long.lv1.data$UTC)]),
